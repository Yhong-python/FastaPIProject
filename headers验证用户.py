...
import uvicorn
from fastapi import FastAPI, Depends, HTTPException, status,Form,Header,Response
...
async def check_jwt_token(token: Optional[str]=Header(default=None)):
...
async def get_current_active_user(user: User = Depends(check_jwt_token)):
    if user.disabled:
...
if __name__ == '__main__':
    uvicorn.run(app="headers验证用户:app", reload=True, host="127.0.0.1", port=8080)